...
namespace SimpleVoidDelegates
{
    class DefiningDelegate
    {
        // delegate <return type> <delegate-name> <parameter list>
        public delegate void operationOf2Integers(int a, int b);
    ...
    public delegate void AnotherDelegate(int a, int b);
    public delegate void DelegateWithOneParam(int a);
    ...
    {        
        public static void Main()
        {
            ...
            AnotherDelegate a2 = delegate(int a, int b) { Console.WriteLine($"Sum: {a} + {b} = {a+b}"); };
            a2?.Invoke(1,1);

            Console.WriteLine();

            // You can also create the delegate by directly declaring a delegate using a lambda expression
            AnotherDelegate a3 = (a,b) => Console.WriteLine($"Sum: {a} + {b} = {a+b}");
            a3?.Invoke(5,4);

            Console.WriteLine();

            // If the lamda expression has just one parameter, you can omit the parameter's parentheses
            DelegateWithOneParam a4 = s => Console.WriteLine($"s squared is: {s} * {s} = {Math.Pow(s,2)}");
            a4?.Invoke(7);
        }
    }
}